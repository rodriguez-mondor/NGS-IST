...
responses_name = st.sidebar.text_input("Nom du fichier de réponses", value=default_resp_name,
                                       help="Sauvegardé dans un répertoire temporaire du serveur.")
# Always write to a guaranteed-writable temp directory
RESPONSES_PATH = os.path.join(tempfile.gettempdir(), responses_name)

st.sidebar.markdown(f"**Chemin d'écriture** : `{RESPONSES_PATH}`")
st.sidebar.caption("La zone temporaire est volatile. Téléchargez vos réponses pour les conserver localement.")
...
if row is None:
    ...
    st.download_button("Télécharger vos réponses", data=load_responses(RESPONSES_PATH).to_csv(index=False).encode("utf-8"),
...
with c2:
    if st.button("💾 Enregistrer et suivant"):
        ...
        if err:
            st.error(f"Erreur d'écriture du fichier de réponses : {err}")
        st.rerun()

with c3:
    st.download_button("⬇️ Télécharger les réponses",
...
with st.expander("Détails techniques (debug)"):
    ...
    st.write(f"Fichier de réponses : {RESPONSES_PATH}")
