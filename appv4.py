...
def load_cases(path_or_buffer):
    ...
    # ensure types
    for c in ['sample_id','filename','patient','visit','matrix','batch_group','pathogen']:
...
with g2:
    ...
    # Common mask for current batch & matrix
    batch_mask = (
        (cases['batch_group'].astype(str) == str(row['batch_group'])) &
        (cases['matrix'].astype(str) == str(row['matrix']))
    )

    # A) complete sample list for the batch (from manifest if provided)
    if manifest is not None:
        batch_samples = (
            manifest.loc[
                (manifest['batch_group'] == str(row['batch_group'])) &
                (manifest['matrix'] == str(row['matrix']))
            , 'sample_id'].astype(str).unique()
        )
    else:
        # Fallback: only samples visible in cases (may undercount zeros)
        batch_samples = cases.loc[batch_mask, 'sample_id'].astype(str).unique()
        st.info("Manifest non fourni : N peut sous-estimer les zeros dans l'histogramme.")

    n_batch = len(batch_samples)

    # B) map of RPMs for the target (only where present in cases)
    ...
    # C) full vector including zeros for missing target
