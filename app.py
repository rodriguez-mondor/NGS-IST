...
st.sidebar.markdown('---')
st.sidebar.subheader('Destination de sauvegarde')
    #
# Infos Google Sheet détectées (debug)
_sid = st.secrets.get('gsheet_id', st.secrets.get('SHEET_ID', ''))
_wname = st.secrets.get('worksheet_name', st.secrets.get('SHEET_TAB', ''))
if _sid:
    st.sidebar.caption(f"Google Sheet ID détecté: **{_sid[:6]}…**")
if _wname:
    st.sidebar.caption(f"Worksheet: **{_wname}**")
if operator_id == 'admin':
    storage_mode = st.sidebar.radio("Où enregistrer ?", ["Google Sheet", "Fichier local (test)"], index=0,
                                    help="Admin peut tester en local ou écrire dans la Google Sheet.")
else:
    storage_mode = "Google Sheet"
    st.sidebar.caption("Enregistrement : **Google Sheet** (imposé pour les opérateurs).")
...
# Infos service account (optionnel)
try:
...
def append_response_gsheet(record: dict):
    """Append one record to Google Sheet using service account in st.secrets.
    Needs:
      - st.secrets["gcp_service_account"]
      - st.secrets["gsheet_id"]
      - optional st.secrets["worksheet_name"] (default 'responses')
    """
    if "gcp_service_account" not in st.secrets:
        raise RuntimeError("st.secrets['gcp_service_account'] absent.")
    ...
    if not sheet_id:
        raise RuntimeError("Sheet ID absent (renseigner 'gsheet_id' ou 'SHEET_ID' dans st.secrets).")
...
with c2:
    if st.button('💾 Enregistrer et suivant'):
        ...
        if storage_mode == "Google Sheet":
            try:
                append_response_gsheet(record)
                st.success("✅ Enregistré dans la Google Sheet.")
            except Exception as e:
