...
def plot_batch_hist(batch_vals, sample_val, species_mode=True, title_suffix='', fs=9):
    ...
    major_labels = ['0.1', '1', '10', '100', '1000', '10000']
...
if operator_id == '— choisir —':
    ...
    st.stop()
# Mode (obligatoire)
_mode = st.sidebar.radio('Choisir le mode :', ['— choisir —','Production','Test'], index=0,
                          help="En mode Test, rien n'est enregistré. En mode Production, c'est écrit dans la Google Sheet.")
if _mode == '— choisir —':
...
default_resp_name = 'responses.csv'
...
# === Google Sheet append (Production) ===
...
# === Progress per operator (assigned in cases + answers in GSheet) ===
assign_cols = [c for c in cases.columns if c.lower() in ('assigned_operator','operator','assigned_to','assignment','assignee','operator_id')]
if len(assign_cols)>0 and operator_id!='admin':
    _assign_col = assign_cols[0]
    cases_for_op = cases[cases[_assign_col].astype(str).str.lower()==operator_id.lower()]
else:
    _assign_col = None
    cases_for_op = cases

total_for_op = int(cases_for_op['question_id'].nunique()) if 'question_id' in cases_for_op.columns else len(cases_for_op)

def _count_answered_gsheet(op_id: str) -> int:
    try:
        import gspread
        from google.oauth2.service_account import Credentials
        creds_info = st.secrets.get("gcp_service_account", None)
        if not creds_info: return 0
        scopes = ["https://www.googleapis.com/auth/spreadsheets.readonly"]
        credentials = Credentials.from_service_account_info(creds_info, scopes=scopes)
        client = gspread.authorize(credentials)
        sheet_id = (st.secrets.get("gsheet_id", "") or st.secrets.get("SHEET_ID", "")).strip()
        if not sheet_id: return 0
        ws_name = st.secrets.get("worksheet_name", st.secrets.get("SHEET_TAB", "responses"))
        sh = client.open_by_key(sheet_id)
        ws = sh.worksheet(ws_name)
        ...
        if 'operator_id' not in df.columns or 'question_id' not in df.columns: return 0
        df = df[df['operator_id'].astype(str).str.lower()==op_id.lower()]
        if _assign_col is not None and 'question_id' in cases_for_op.columns:
            assigned_qids = set(cases_for_op['question_id'].astype(str).tolist())
            df = df[df['question_id'].astype(str).isin(assigned_qids)]
        return int(df['question_id'].astype(str).nunique())
    except Exception:
        return 0

answered_for_op = _count_answered_gsheet(operator_id) if _mode=='Production' else 0
...
pc2.metric('Vos reponses', f'{answered_for_op}/{total_for_op}')
...
with c2:
    if st.button('💾 Enregistrer et suivant'):
        ...
        if _mode == 'Production':
            ...
            st.info('🧪 Mode Test : aucune écriture (ni locale, ni Google Sheet).')
        clear_current_row()
        st.rerun()
