...
def plot_batch_hist(batch_vals, sample_val, species_mode=True, title_suffix='', fs=9):
    # Histogramme (mêmes données & bins), mais axe X en log et borné à 1000 (modif esthétique uniquement)
    edges = [0, 1, 10, 100, 1000, 10000]  # inchangé
    fig, ax = plt.subplots(figsize=(6.2, 3.2))
    ax.hist(batch_vals, bins=edges)  # inchangé
    # Ligne verte (échantillon) — inchangé
    ax.axvline(sample_val, color='green', linestyle='-', linewidth=2, label='Sample')
    # Repères aux seuils 0/1/10 — inchangé
    for thr in [0, 1, 10]:
        ax.axvline(thr, color='black', linestyle=':', linewidth=1)
    # === Seule modif demandée : axe X en log et borné ===
    ax.set_xscale('symlog', linthresh=0.1)   # permet d'afficher 0 proprement
    ax.set_xlim(0, 1000)                     # borne max fixée à 1000
    ax.set_xticks([0, 0.1, 1, 10, 100, 1000])
    ax.get_xaxis().set_major_formatter(plt.ScalarFormatter())
    # Libellés — seule mention "log RPM" pour refléter l'échelle
