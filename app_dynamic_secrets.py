...
# =========================
# Config helpers (read every run)
# =========================
def get_cfg():
    return {
        "operators": [x.strip() for x in st.secrets.get("OPERATORS", "op1,op2").split(",") if x.strip()],
        "admins": [x.strip() for x in st.secrets.get("ADMIN_OPERATORS", "admin").split(",") if x.strip()],
        "pins": json.loads(st.secrets.get("OPERATOR_PINS_JSON", "{}")) if "OPERATOR_PINS_JSON" in st.secrets else {},
        ...
        "long_base_path": st.secrets.get("LONG_BASE_PATH", ""),
...
# =========================
@st.cache_resource
def get_gsheet_ws(sheet_id, sheet_tab, service_email):
    """Ouvre/initialise la feuille Google Sheets (onglet responses)."""
    scopes = ["https://www.googleapis.com/auth/spreadsheets"]
    creds = Credentials.from_service_account_info(dict(st.secrets["gcp_service_account"]), scopes=scopes)
    gc = gspread.authorize(creds)
    sh = gc.open_by_key(sheet_id)  # lève SpreadsheetNotFound si ID invalide ou non partagé
    try:
...
def save_to_gsheet(cfg, d):
    ws = get_gsheet_ws(cfg["sheet_id"], cfg["sheet_tab"], cfg["service_email"])
...
st.title("NGS-IST — Annotation operateur")
...
with st.sidebar:
    st.header("Connexion")
    op = st.selectbox("Identifiant", options=cfg["operators"] + cfg["admins"], index=0)
    pin_needed = cfg["pins"].get(op)
    if pin_needed:
    ...
    TEST_MODE = st.checkbox("Mode TEST (local)", value=False) if op in cfg["admins"] else False

    if op in cfg["admins"]:
        if st.button("Reinitialiser le cache (Google Sheets)"):
            st.cache_resource.clear()
            st.cache_data.clear()
            st.success("Caches vides. Relancez la page si besoin.")

        if st.button("Lister cles des secrets (non sensibles)"):
            keys = [k for k in st.secrets.keys() if k != "gcp_service_account"]
            st.write("Top-level keys:", keys)
            st.write("SHEET_ID present ?", "SHEET_ID" in st.secrets)
            sid = cfg["sheet_id"]
            sid_mask = (sid[:6] + "…" + sid[-4:]) if sid else "(vide)"
            st.write(f"SHEET_ID (masque): {sid_mask}")
            st.write(f"SHEET_TAB: {cfg['sheet_tab']}")
            st.write(f"Service account: {cfg['service_email']}")

        if st.button("Afficher debug & tester ouverture Google Sheets"):
            try:
                st.info(f"SHEET_ID: `{cfg['sheet_id']}` | SHEET_TAB: `{cfg['sheet_tab']}` | service_account: {cfg['service_email']}")
                ws_dbg = get_gsheet_ws(cfg["sheet_id"], cfg["sheet_tab"], cfg["service_email"])
                st.success("Connexion OK : feuille accessible et onglet pret.")
                try:
                    st.write("Feuilles disponibles:", [w.title for w in ws_dbg.spreadsheet.worksheets()])
                except Exception as e2:
                    st.warning(f"Ouverture OK mais listing des onglets a echoue: {e2}")
            except Exception as e:
                st.error(f"Echec d'ouverture: {e!r}")

    if op in cfg["admins"] and st.button("Tester connexion Google Sheets (ecriture ligne test)"):
        try:
            ws = get_gsheet_ws(cfg["sheet_id"], cfg["sheet_tab"], cfg["service_email"])
            ...
            st.success("OK : ligne test ecrite dans la Google Sheet ✅")
        except Exception as e:
            st.error(f"Erreur : {e!r}")

# Progression
answered_ids = answered_ids_for(cfg, op) if not TEST_MODE else set()
...
if row is None:
    st.success("🎉 Vous avez termine toutes vos questions pour cet identifiant.")
    st.stop()
...
st.markdown("---")
st.subheader("Votre interpretation")
...
if btn:
    if not label:
        st.warning("Veuillez selectionner un label.")
        st.stop()
    ...
    try:
        if TEST_MODE:
            ...
            if not cfg["sheet_id"]:
                ...
                st.stop()
            save_to_gsheet(cfg, out)
        st.success("Reponse enregistree ✅")
        st.cache_data.clear()
        st.experimental_rerun()
    except Exception as e:
        st.error(f"Erreur d’enregistrement : {e!r}")
