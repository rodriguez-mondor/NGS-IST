...
def plot_batch_hist(batch_vals, sample_val, species_mode=True, title_suffix='', fs=9):
    # Histogramme avec barres couvrant entièrement les intervalles [0–1], (1–10], (10–100], (100–1000], (≥1000)
    edges = [0, 1, 10, 100, 1000, 10000]
    fig, ax = plt.subplots(figsize=(6.2, 3.2))
    ax.hist(batch_vals, bins=edges)  # barres pleines aux bornes définies
    # Ligne verte = valeur de l'échantillon
    ax.axvline(sample_val, color='green', linestyle='-', linewidth=2, label='Sample')
    # Repères aux seuils
    for thr in [0, 1, 10]:
        ax.axvline(thr, color='black', linestyle=':', linewidth=1)
    # Limite x intelligente
    xmax = max(10, sample_val * 1.2, max(edges))
    ax.set_xlim(0, xmax)
