...
with g2:
    ...
    if manifest_stats is None:
        ...
        if subset.empty:
            ...
            # Histogramme par INTERVALLES pleins (0–1, 1–10, 10–100, 100–1000, ≥1000) à partir des bins du manifest
            stats_row = subset.iloc[0]
            cols_bins = ['count_0_1','count_1_10','count_10_100','count_100_1000','count_ge_1000']
            c = subset[cols_bins].sum() if len(subset) > 1 else stats_row[cols_bins]
            heights = [int(c.get('count_0_1',0)), int(c.get('count_1_10',0)), int(c.get('count_10_100',0)),
                       int(c.get('count_100_1000',0)), int(c.get('count_ge_1000',0))]
            ...
            # Ligne verte = valeur de l'échantillon courant
            ax.axvline(float(row[used_col]), color='green', linestyle='-', linewidth=2)
            # Limites X
            ...
            ax.set_xlim(0, xmax)
            n_batch = int(stats_row.get("n_total", sum(heights)))
            title_sfx = f"({row['matrix']}, {row['pathogen']}, N={n_batch})"
            ax.set_xlabel(f"RPM ({'species' if species_mode else 'genus'})", fontsize=9)
            ax.set_ylabel('Count', fontsize=9)
            ax.set_title(f"Batch distribution " + title_sfx, fontsize=9)
