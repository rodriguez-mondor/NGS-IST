...
def get_gsheet_ws(sheet_id, sheet_tab, service_email):
    ...
    NOTE: on passe sheet_id/sheet_tab/service_email comme arguments
    pour que le cache se ré-invalide si l'un change.
    """
...
@st.cache_data
def load_manifest():
    """Charge le manifest agregé (optionnel) pour le graph de distribution batch."""
...
def save_to_gsheet(d):
    ws = get_gsheet_ws(SHEET_ID, SHEET_TAB, st.secrets["gcp_service_account"]["client_email"])
...
def next_pending(my_cases, answered_ids):
    """Donne la prochaine question en attente pour l’operateur."""
...
with st.sidebar:
    ...
    # Mode TEST visible uniquement pour admin (ecrit en local /tmp)
    TEST_MODE = st.checkbox("Mode TEST (local)", value=False) if op in ADMIN_OPERATORS else False

    # Boutons admin: debug et reset cache
    if op in ADMIN_OPERATORS:
        ...
        if st.button("Afficher debug & tester ouverture Google Sheets"):
            try:
                ...
                st.info(f"SHEET_ID: `{SHEET_ID}` | SHEET_TAB: `{SHEET_TAB}` | service_account: {service_email}")
                ws_dbg = get_gsheet_ws(SHEET_ID, SHEET_TAB, service_email)
    ...
    if op in ADMIN_OPERATORS and st.button("Tester connexion Google Sheets (ecriture ligne test)"):
        try:
            ws = get_gsheet_ws(SHEET_ID, SHEET_TAB, st.secrets["gcp_service_account"]["client_email"])
...
# En mode TEST avec un identifiant admin, il est normal de ne rien avoir a annoter (non assigne a admin).
...
with c1:
    ...
    # used species vs genus selon la regle
    if str(row.get("species_or_spp", "species")) == "species":
        used_rpm = float(row.get("rpm_species", 0.0))
        used_ctrl = float(row.get("ctrl_env_rpm_species", 0.0))
    else:
        used_rpm = float(row.get("rpm_genus", 0.0))
        used_ctrl = float(row.get("ctrl_env_rpm_genus", 0.0))
...
with c2:
    ...
    colors = ["#2ca02c", "#2ca02c", "#d62728", "#d62728"]  # vert espece, rouge genre
    ...
    if manifest is not None:
        ...
        if not row_m.empty:
            ...
            # Axe secondaire pour les vlines RPM (0→1000+)
            ax2t = ax2.twiny()
    ...
    if not have_graph2 and long_base is not None:
        ...
        if not dfB.empty:
            # used = rpm_species (espece) sauf "... spp" & "Papillomavirus" (genre)
            def _u(rrow):
                p = str(rrow["pathogen"])
...
if btn:
    ...
    try:
        ...
        st.cache_data.clear()  # rafraichir progression
