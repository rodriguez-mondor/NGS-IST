...
def load_manifest_batch_stats():
    """Charge d'abord le manifest local, puis fallback URL GitHub.
    Normalise les colonnes pour compatibilité avec le code en aval.
    """
    local_path = "manifest_batch_pathogen_stats_from_raw.csv"
    df = None
    try:
        if os.path.exists(local_path):
            df = pd.read_csv(local_path)
    except Exception as e:
        st.warning(f"Manifest local introuvable/illisible: {e}")
    if df is None:
        try:
            df = pd.read_csv(MANIFEST_BATCH_STATS_URL)
        except Exception as e:
            st.warning(f"Impossible de charger manifest_batch_stats depuis GitHub : {e}")
            return None
    # --- Normalisations ---
    ...
    if 'batch_group' not in df.columns:
        if {'batch_id','batch_matrix'}.issubset(df.columns):
...
def plot_batch_hist(batch_vals, sample_val, species_mode=True, title_suffix='', fs=9):
    ...
    ax.hist(batch_vals, bins=20)  # default color
...
operator_id = st.sidebar.selectbox('Votre identifiant operateur', ['op1','op2','op3','op4','op5','admin'], index=0)
...
def load_cases(path_or_buffer):
    ...
    for c in ['sample_id','filename','patient','visit','matrix','site','batch_group','pathogen']:
        df[c] = df[c].astype(str)
...
# Load precomputed batch stats from GitHub
...
# Histogramme par INTERVALLES pleins, pas par midpoints
c = stats_row[['count_0_1','count_1_10','count_10_100','count_100_1000','count_ge_1000']]
heights = [int(c.get('count_0_1',0)), int(c.get('count_1_10',0)), int(c.get('count_10_100',0)), int(c.get('count_100_1000',0)), int(c.get('count_ge_1000',0))]
edges = [0, 1, 10, 100, 1000, 10000]
left = edges[:-1]
width = np.diff(edges)
fig, ax = plt.subplots(figsize=(6,3))
ax.bar(left, heights, width=width, align='edge', alpha=0.6)
# repères
ax.axvline(1, linestyle=':', linewidth=1)
ax.axvline(10, linestyle=':', linewidth=1)
# ligne verte = valeur echantillon
ax.axvline(float(row[used_col]), color='g', linewidth=2)
# limites intelligentes
max_edge_with_counts = max([e for e,h in zip(edges[1:], heights) if h>0] + [10])
xmax = max(10, float(row[used_col])*1.2, max_edge_with_counts)
ax.set_xlim(0, xmax)
ax.set_xlabel(f"RPM ({'species' if species_mode else 'genus'})", fontsize=9)
ax.set_ylabel('Count', fontsize=9)
ax.set_title(f"Batch distribution ({row['matrix']}, {row['pathogen']}, N={int(stats_row.get('n_total', sum(heights)))} )", fontsize=9)
