...
def _compute_batch_fallback(cases: pd.DataFrame, row: pd.Series):
    """Retourne (arr, sample_val, species_mode, n_pos, n_total).
    - spp & Papillomavirus => agrégation par genre sur rpm_genus via contains/match
    - espèce => rpm_species sur correspondance exacte
    - batch = même batch_group & matrix, hors contrôles (CTRL/CTRENV/CIQ)
    """
    import re
    name = str(row.get("pathogen", "")).strip()
    is_hpvcoll = ("papillomavirus" in name.lower())
    is_spp = name.endswith(" spp") or is_hpvcoll
    species_mode = not is_spp
    used_col = "rpm_species" if species_mode else "rpm_genus"
    sample_val = float(row.get(used_col, 0.0) or 0.0)

    # batch frame (hors contrôles)
    mask_batch = (cases["batch_group"].astype(str) == str(row.get("batch_group"))) & (cases["matrix"].astype(str) == str(row.get("matrix")))
    is_ctrl = cases["filename"].astype(str).str.contains(r"CTRL|CTRENV|CIQ", case=False, na=False)
    batch_df = cases.loc[mask_batch & ~is_ctrl, ["filename","pathogen","rpm_species","rpm_genus"]].copy()

    if is_spp:
        if is_hpvcoll:
            sel = batch_df["pathogen"].astype(str).str.contains("papillomavirus", case=False, na=False)
        else:
            g = name.split()[0]
            sel = batch_df["pathogen"].astype(str).str.match(rf"^{re.escape(g)}\s", na=False) | batch_df["pathogen"].astype(str).str.match(rf"^{re.escape(g)}\sspp$", na=False)
        sub = batch_df.loc[sel]
        if not sub.empty:
            arr = sub.groupby("filename")[used_col].max().fillna(0).to_numpy()
        else:
            arr = np.array([])
    else:
        sub = batch_df.loc[batch_df["pathogen"].astype(str) == name]
        if not sub.empty:
            arr = sub.groupby("filename")[used_col].max().fillna(0).to_numpy()
        else:
            arr = np.array([])

    n_total = int(batch_df["filename"].nunique())
    n_pos = int((arr > 0).sum())
    return arr, sample_val, species_mode, n_pos, n_total
# ---------------------------
...
with g2:
    ...
    if manifest_stats is None:
        ...
        arr, sample_val, species_mode, n_pos, n_batch = _compute_batch_fallback(cases, row)
        title_sfx = f"({row['matrix']}, {row['pathogen']}, N={n_pos}/{n_batch})"
        plot_batch_hist(arr, sample_val, species_mode=species_mode, title_suffix=title_sfx, fs=9)
    else:
