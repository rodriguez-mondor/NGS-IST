...
_mode = st.sidebar.radio("Choisir le mode :", ["— choisir —", "Production", "Test"], index=0,
                         help="En mode Test, rien n'est enregistré. En mode Production, l'enregistrement va automatiquement dans la Google Sheet.")
if _mode == "— choisir —":
...
def append_response_gsheet(record: dict):
    ...
    if not sheet_id:
        raise RuntimeError("Sheet ID absent (renseigner 'gsheet_id' ou 'SHEET_ID').")
    ...
    if st.button('💾 Enregistrer et suivant'):
        ...
        if _mode == "Production":
            try:
                ...
                st.success("✅ Enregistré (mode Production) dans la Google Sheet.")
            except Exception as e:
