...
with g2:
    ...
    if manifest_stats is None:
        ...
        try:
                ...
                if n_pos == 0:
                    n_pos = int(stats_row.get('count_0_1',0)) + int(stats_row.get('count_1_10',0)) + int(stats_row.get('count_10_100',0)) + int(stats_row.get('count_100_1000',0)) + int(stats_row.get('count_ge_1000',0))
            except Exception:
                n_pos = int(stats_row.get('count_0_1',0)) + int(stats_row.get('count_1_10',0)) + int(stats_row.get('count_10_100',0)) + int(stats_row.get('count_100_1000',0)) + int(stats_row.get('count_ge_1000',0))
            title_sfx = f"({row['matrix']}, {row['pathogen']}, N={n_pos}/{n_batch})"
        plot_batch_hist(batch_vals, float(row[used_col]), species_mode=species_mode, title_suffix=title_sfx, fs=9)
