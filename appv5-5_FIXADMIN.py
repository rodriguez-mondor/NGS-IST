...
def assigned_set_for_operator(df: pd.DataFrame, assign_col: str, op_id: str) -> set:
    ...
    mask = ser.apply(lambda v: any(op_id.lower()==tok.strip().lower() for tok in v.split(',') if tok.strip()))
    return set(df.loc[mask, 'question_id'].astype(str).tolist())
...
if operator_id == 'admin':
    if 'admin_mode' in locals() and admin_mode == 'Supervision':
        responses = load_responses_gsheet()
    else:
        responses = pd.DataFrame()
else:
    if '_mode' in locals() and _mode == 'Production':
        responses = load_responses_gsheet()
    else:
        responses = pd.DataFrame()
...
# === Progress & filtering per role ===
questions_version = compute_questions_version(cases)

assign_col = detect_assignment_column(cases)
if operator_id == 'admin':
    if 'admin_mode' in locals() and admin_mode == 'Supervision':
        ops = [op for op in ['op1','op2','op3','op4','op5'] if True]
        def assigned_for(op):
            return len(assigned_set_for_operator(cases, assign_col, op))
        ...
        if assign_col:
            for op in ops:
                assigned_unique |= assigned_set_for_operator(cases, assign_col, op)
        covered_unique = set()
        ...
        cA, cB = st.columns(2)
        cov_pct = f"{len(covered_unique)}/{len(assigned_unique) if assigned_unique else 0}"
        chg_pct = f"{total_answered_global}/{total_assigned_global}"
        cA.metric('Couverture unique (questions avec ≥1 réponse)', cov_pct)
        cB.metric('Charge complétée (avec redondance)', chg_pct)
...
if operator_id!='admin' and '_mode' in locals() and _mode=='Production' and not responses.empty:
    ...
    already = set()
# legacy metrics removed (handled above)
